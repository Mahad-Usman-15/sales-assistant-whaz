...
*   **Body (white, `#FFFFFF`):** Contains the dynamic proposal content. A large, low-opacity gray watermark (a stylized "Z" behind a diagonal arrow and a four-point starburst) sits in the lower-right of the body — decorative only, must never sit under or collide with body text/service lists.
    *   ⚠️ **Not implemented in v1.** No exported asset for this watermark exists (`docs/header.png` and `docs/footer.png` contain only their bands — verified: fully transparent outside them). It is deferred out of the v1 proposal generator by explicit decision (see `specs/001-proposal-pdf-generator/spec.md`, Clarifications 2026-07-19) and is not part of that feature's fidelity bar. To reinstate it, export a transparent PNG of the watermark and update FR-005.
...
*   **Implementation guidance — the bands are built in CSS, not shipped as images.** The reference PNGs in `docs/` were produced by an AI image generator; **there is no vector/editable source file**, so they cannot be re-exported at a higher resolution and no upscaler can recover the missing detail. `docs/header.png` and `docs/footer.png` are therefore **visual reference only** — never bundled into the app.
    *   ⚠️ Earlier guidance in `projectplan.md` §9 (and an earlier revision of this section) said to treat the bands as pre-exported high-resolution image assets. **That is superseded** — it assumed an editable source file that does not exist. See `specs/001-proposal-pdf-generator/research.md` R1.
    *   The composition maps onto ordinary CSS primitives: `linear-gradient` band (`#111111` → `#0a0436`), `clip-path: polygon(...)` diagonal cut (mirrored between header and footer), repeating `radial-gradient` dot grid, `border-left` divider rules, inline SVG for the social/envelope icons and the star accents, and the **WHAZ wordmark set in Montserrat Black (900)** — the same self-hosted face used elsewhere in the document.
    *   Benefits over the raster: sharp at any zoom or print size, and the contact email/taglines become real selectable, searchable text rather than pixels.
*   **Reference geometry (from the AI-generated PNGs, measured):** the artwork occupies header rows 607–883 and footer rows 664–827 of a padded 1054 × 1492 canvas — i.e. bands of **55.2 mm** and **32.7 mm** page height respectively at A4 width. Preserve those proportions when building the CSS.