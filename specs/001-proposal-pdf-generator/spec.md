...
- **Very long free-text notes** that exceed one page: content flows onto additional pages with the letterhead header/footer chrome present on every page, never truncated silently.
- **Empty optional fields** (no notes, no services selected): the proposal generates cleanly, omitting empty sections rather than showing blank labels.
...
- **FR-002**: The system MUST let the rep select zero or more services from the Whaz catalog defined in `tools.md`, displaying each service's name and short description exactly as written in that source.
- **FR-003**: The system MUST provide a free-text notes/benefits field whose content is placed into the proposal body verbatim.
...
- **FR-005**: The generated PDF MUST reproduce the Whaz letterhead chrome — header band and footer band, including their diagonal cuts, gradient, wordmark, contact/tagline zones, and dot-grid texture — to visually match the reference material (`docs/header.png`, `docs/footer.png`, `docs/letterhead-skeleton.jpeg`, `docs/reference-letter-head.pdf`, `design.md` §5) with no perceptible drift. The chrome MUST remain sharp at any zoom or print size, and text within it (contact email, taglines, wordmark) MUST be real selectable text rather than rasterised pixels. The decorative body watermark described in `design.md` §5 is **deferred out of v1** (no asset exists; see Clarifications 2026-07-19) and is not part of this feature's fidelity bar.
...
- **FR-014**: The proposal body MUST be laid out as a full client letter with these fixed sections in order: a salutation addressed to the recipient, a short introduction line, the selected-services section, the free-text notes/benefits section, a closing line, and a sender signature block (the prepared-by name). Optional sections (services, notes) are omitted cleanly when empty; the salutation, closing, and signature block are always present.
...
- **Proposal Draft**: The transient set of rep-entered values for one generation — client/recipient details, sender details, selected services, free-text notes, and generation date. Exists only for the duration of a single request; not stored.
- **Service Catalog Item**: A single Whaz offering from `tools.md` — its name, short description, and free/paid category. Read-only reference data for v1.
...
- **Confirmed field set (FR-001)**: The MVP form captures — Recipient/client name, Client company/organization, Proposal title or subject, Prepared-by (rep) name, Proposal date (defaulting to today), plus the optional services selection and notes field. Confirmed via clarification (2026-07-17); recipient-side contact fields are intentionally excluded since Whaz's contact lives in the letterhead footer.
...
- **Constitution**: Bound by `.specify/memory/constitution.md` **v2.0.0** — deterministic rendering (zero AI in the render path), brand fidelity as ground truth (chrome built in CSS/SVG, reference rasters never bundled, fidelity verified by side-by-side review), stateless-by-default, and the $0 cost ceiling all apply directly to this feature.
