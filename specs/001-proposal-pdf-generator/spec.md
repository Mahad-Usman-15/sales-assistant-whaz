...
- **Constitution**: Bound by `.specify/memory/constitution.md` **v3.0.0** — deterministic rendering (zero AI in the render path), brand fidelity as ground truth (the approved letterhead raster is embedded rather than reconstructed, never upscaled or regenerated, with fidelity verified numerically to a 0.5mm baseline tolerance), stateless-by-default, and the $0 cost ceiling all apply directly to this feature.
