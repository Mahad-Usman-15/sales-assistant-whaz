...
|---|---|---|
| **0 — Fix `/tmp` exhaustion** *(hard prerequisite)* | Memoised module-scope `getBrowser()`; per-request `newContext()`/close in `finally`, never the browser; `isConnected()` self-heal; concurrency semaphore (~2) for Fluid Compute | 20 sequential generations on one instance, all 200 (**SC-009**) |
