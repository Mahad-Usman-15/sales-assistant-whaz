...
Engineering notes for the Whaz Proposal Generator. For what the product does and who it's for, see
...
`app/globals.css` styles the **browser form only** and shares nothing with the PDF. Its tokens come
from `WEB_DESIGN.md`; the document's own type and layout come from the reference PDF. The two answer
to different references — never propagate a value between them.
...
curl -X POST localhost:3000/api/generate -H 'Content-Type: application/json' \
  -d '{"recipientName":"A","recipientRoleLine1":"B","clientCompany":"C","proposalDate":"2026-07-26","selectedServiceIds":[]}' \
  -o out.pdf -w '%{http_code} %{content_type}\n'
```

A `200 application/pdf` there with a broken page means hydration, not rendering.

⚠️ **The integration suite runs against a production build** (`npm run build && npm start`), because
`next dev` proved unreliable under sustained PDF rendering. It therefore does **not** cover
dev-mode-only breakage — the hydration bug above passed all 13 tests. After touching a client
component or an app-router file, load the page in a real browser before calling it working.
...
drift. Edit `tools.md`, then update `lib/catalog.ts` until that test passes, then run
`npm run check:table-fit` — table cells must stay on one line or the row pitch breaks.
