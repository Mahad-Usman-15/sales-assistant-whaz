...
- **The app is implemented and passing tests.** Feature 001 is complete except **T062** (deploy to Vercel + re-run the quickstart against the deployed URL) — T059/T061/T063 are all `[X]`; the "T001–T060 of 63" line that stood here until 2026-08-05 was wrong. Feature 002 is at **90/104** (see below). Feature 002's T102 folds in 001's T062.
- **Feature 002 (RBAC) is largely built and deployed on preview.** All three user stories are implemented and verified: sign-in gates every surface including `POST /api/generate`; Admin/Sales roles with invitation-only access, reversible removal and the last-Admin guarantee; per-member and org-wide generation counts. Remaining work is Phase 6 polish — accessibility (T008/T095/T096), the invite-expiry sweep (T094), docs (T097/T099), and release (T101–T103). Last full run: **33/33 Playwright · 94/94 unit · tsc · lint clean**, against a production build.
...
- **Node runtime required everywhere, never Edge** — `playwright-core` + `@sparticuz/chromium` don't run on Edge, and neither does the Postgres driver. Next 16's `proxy` runtime is fixed to `nodejs` and is **not configurable**. (`plan.md` says Next 15 — 16 was used per explicit user instruction to take the latest; check `package.json` for exact current versions.)
- **Auth/data stack** (feature 002): Supabase Auth passwordless magic link + Supabase Postgres via **Prisma 7 driver adapters** (`@prisma/adapter-pg`) through the Supavisor **transaction** pooler. Dashboard UI is Tailwind v4 + vendored Tremor Raw, scoped so it cannot reach `app/globals.css` (ADR-0004).
- **Key commands**: `npm run dev` · `npm run dev:clean` · `npm run build` (⚠️ runs `prisma generate` first — omitting it fails the Vercel build) · `npm run test:unit` · `npm run test:integration` · `npm run test:db` (real-Postgres concurrency suite) · `npm test` · `npm run check:table-fit` · `npm run check:render-scale` · `npm run check:email <address>` · `npm run check:deployed-renders <url>`. First run also needs `npx playwright install chromium`.
- ⚠️ **`.env` is not auto-loaded by Prisma 7** — `prisma.config.ts` loads `.env.local` then `.env` explicitly, or `prisma migrate` sees no connection string. `DIRECT_URL` must be the **session pooler `:5432`**, never `db.<ref>.supabase.co` (IPv6-only; migrations fail `ENETUNREACH` from Vercel's IPv4 build containers).
...
- `scripts/` — `preview-chrome.mjs` (screenshots the preview docs), `check-table-fit.mjs` (`npm run check:table-fit`; fails if catalog copy would wrap a table cell), `check-email.mjs` (sends one real magic link; Supabase hides SMTP errors from clients, so the authoritative reason is in Dashboard → Logs → Auth Logs), `check-redirect-urls.mjs` (probes the Supabase redirect allow-list via `generateLink` **without sending mail** — an unlisted redirect is silently replaced by Site URL, which logs nothing anywhere), `check-deployed-renders.mjs` (drives sequential + concurrent renders against a deployed URL for SC-009), and ⚠️ **`bootstrap-admin.ts` — an operator script that must never be deployed or referenced from any `package.json` build/postinstall hook** (FR-036). It refuses to run once an active Admin exists.
- `server/` — **server-only code; deliberately not `lib/`.** `db/client.ts` (Prisma singleton, `import 'server-only'` on line 1), `auth/{supabase,admin,guard}.ts`, `repo/{users,invitations,generations}.ts`, `errors.ts`. ⚠️ **`server/auth/guard.ts` is THE authorization boundary** — `requireUser()`/`requireAdmin()`, React `cache()`-wrapped, re-reading role/status from `app_user` every request. ⚠️ **`server/auth/admin.ts` is the only module permitted to read `SUPABASE_SECRET_KEY`.** Repo functions take the actor as a typed first parameter, and `AdminActor` is constructible only by `requireAdmin()` — a missing check is a compile error, not a review miss.
- `proxy.ts` (repo root) — ⚠️ **not `middleware.ts`**; that file does not exist in Next.js 16 and the build throws if both are present. Session refresh and redirect only, **never** an authorization decision. Its matcher deliberately excludes `/api`: routes answer with status codes, pages redirect.
- `prisma/` — `schema.prisma` (generator `prisma-client`, mandatory `output`) and five migrations: RBAC schema + `citext`; the cross-schema FK to `auth.users` plus the `handle_new_auth_user` trigger and last-admin backstop; trigger scoping; the `citext` comparison fix; nullable `invited_by_id`. `prisma.config.ts` owns the connection config — ⚠️ Prisma 7 **rejects** `url`/`directUrl` in `schema.prisma` with P1012. `generated/prisma/` is gitignored and produced by `npm run build`.
- `components/dashboard/` — vendored Tremor Raw plus `utils.ts` (`cx`, `focusInput`, `hasErrorInput`). ⚠️ **Do not create `lib/utils.ts`** — pasted Tremor components import from `@/lib/utils` by default and the path must be adjusted on paste, because `lib/` is declared framework-free and reserved for the render pipeline.
...
- `specs/<feature>/{spec,plan,tasks}.md` — created per-feature by `/sp.specify` → `/sp.plan` → `/sp.tasks`.
  ...
  - `specs/002-rbac-dashboard/` — `spec.md`, `plan.md`, `research.md`, `data-model.md`, `contracts/auth-and-admin.md`, `quickstart.md`, `tasks.md`, `checklists/requirements.md` (2026-08-01, **104 tasks T001–T103 + T014a; 90 done as of 2026-08-05 — all three user stories implemented and verified, Phase 6 polish outstanding**). ⚠️ **T011–T014a are Supabase dashboard configuration, not code** — they produce no diff and are the steps most likely to be skipped; each carries an explicit verification, and T014a's is `node scripts/check-redirect-urls.mjs <member-address>`. Sign-in gating the whole app, two roles (Admin/Sales), invitation-only access, reversible removal, the at-least-one-active-Admin guarantee, and per-member / org-wide generation counts. 45 FRs, 14 SCs, 14 assumptions, 6 clarifications. Source: `dashboard.txt`. Constitution Check **PASS against v4.0.1**. Three findings worth knowing before touching this: **FR-042 (sign-in throttling) is satisfied by Supabase config, not code** (`otp.period` per address + `otp.requests_per_hour`); **FR-041's 30-day session had to move into application code** because Supabase's session controls are Pro-only and buying them breaches Principle V; and **FR-043 forces `shouldCreateUser: true`** — the intuitive `false` makes the provider error on unknown addresses, turning the sign-in form into a staff enumerator. ⚠️ Audience note: `whaz.md`'s "Target Audience" describes Whaz's *clients* and is **not** this tool's users — internal-only, two roles, no client-facing surface.
    - **Open findings from `/sp.analyze` (2026-08-03), carried here so they are not lost with the report:** ⚠️ **SC-009 still says 20 consecutive generations; the stated requirement is 100 minimum** — the code clears 100, the criterion is what is stale. **FR-043 has no test** (identical response and no timing difference for member vs non-member) — the one requirement whose violation is silent. **FR-011 is knowingly partial** — a forbidden route answers 200-with-404-content while a missing one answers 404, so existence is inferable; pinned as a KNOWN GAP test in `tests/integration/admin-access.spec.ts`, and *not* a constitution violation (Principle VI has no existence-disclosure clause, contrary to `spec.md`'s Dependencies note). **Sign-in currently sends as `mahadusmn07@gmail.com`, a personal account**, though `spec.md` names `whazpk@gmail.com` — change before rollout. Also stale: spec pins constitution v4.0.0 (now v4.0.1) and `Status: Draft`; PHRs `0011+` need backfilling.
...
- **Env vars are now required** (this changed with feature 002 — the note here previously said the app needed none). Five: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SECRET_KEY`, `DATABASE_URL` (transaction pooler `:6543`, `pgbouncer=true`), `DIRECT_URL` (session pooler `:5432`). They live in `.env.local`/`.env`, both untracked, never hardcoded. ⚠️ `SUPABASE_SECRET_KEY` is confined to `server/auth/admin.ts` behind `import 'server-only'` and **never** carries a `NEXT_PUBLIC_` prefix — that prefix inlines a value into the client bundle at build time, which for this key would publish full database access. ⚠️ `.gitignore`'s `/.env*` also excludes `.env.example`, so no env documentation is committed; this is a deliberate choice for an internal tool (2026-08-05), not an oversight — add `!/.env.example` if that ever changes.
